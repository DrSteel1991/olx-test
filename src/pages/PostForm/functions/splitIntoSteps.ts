...
export const splitIntoSteps = (categoryFields: CategoryFieldConfig["flatFields"] | undefined) => {
    console.log("categoryFields", categoryFields)
    const filteredCategoryFields =
        categoryFields?.filter((field) => field.groupIndex !== null && !field.roles.includes("exclude_from_post_an_ad")) ?? []

    console.log("filteredCategoryFields", filteredCategoryFields)
